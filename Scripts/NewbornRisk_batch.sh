...
SCRIPTS='/mnt/storage_pool/Genomics/Genome/NewbornRisk'
...
# remove missing ALT
bcftools view -e 'ALT = "."' $VCF -Oz -o $OUTPUT_DIR/${INPUT_SAMPLE}_rmmissingalt.vcf.gz --threads 20
# split into biallelic
bcftools norm -m -both -Oz -o $OUTPUT_DIR/${INPUT_SAMPLE}_biallelic.vcf.gz $OUTPUT_DIR/${INPUT_SAMPLE}_rmmissingalt.vcf.gz --threads 20
# remove the duplicated variant
bcftools norm -d exact -Oz -o $OUTPUT_DIR/${INPUT_SAMPLE}_biallelic_nodup.vcf.gz $OUTPUT_DIR/${INPUT_SAMPLE}_biallelic.vcf.gz --threads 20

# Step 2: VEP
echo "$(date) Run VEP for $INPUT_SAMPLE"
conda run -n vep114 bash $SCRIPTS/vep.sh \
    -v $OUTPUT_DIR/${INPUT_SAMPLE}_biallelic_nodup.vcf.gz \
    -o $OUTPUT_DIR/${INPUT_SAMPLE}_vep.vcf.gz \
    -i $INPUT_SAMPLE \
    -g $GENOME \
    --only_pass $ONLY_PASS
...
conda run -n vep python "$SCRIPTS_DIR/batch.py" \
    "$OUTPUT_DIR/${INPUT_SAMPLE}_vep.vcf.gz" \
    "$OUTPUT_DIR/${INPUT_SAMPLE}.txt" \
    "$OUTPUT_DIR/${INPUT_SAMPLE}.ped" \
    "$AF_CLINVAR" \
    "$AF_PRECITION" \
    "$ADA" "$RF" \
    "$REVEL" \
...
conda run -n varxomics Rscript $SCRIPTS/Batch_Analysis.R $INPUT_SAMPLE $OUTPUT_DIR/${INPUT_SAMPLE}.txt $META_DATA $OUTPUT_DIR $GENEDB 
